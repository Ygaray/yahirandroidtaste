...
`v1.11.1` was cut in **SecondBrain v4.0 Phase 155 Plan 12** (`REMIND-09` / `G-155-3`), a patch
directly on top of `v1.11.0` on the same `v1.10.0`-forked branch (still deliberately bypassing
`v2.0.0` — the branch-topology deviation above is unchanged, not re-litigated). The tag closes a
gap `v1.11.0` left behind: `TextCard.kt` and `AlbumCard.kt` still nulled their entire
`headerContent` lambda whenever `titleSlotVisible(title)` was false, so a title-less Text or Album
card dropped its whole header cluster — including the just-added `ReminderIndicator` — even when
the card was pinned, favorited, had images (Text only), or had a reminder. `ListCard.kt` and
`VoiceCard.kt` already widened this guard with an OR on their own content; this tag applies the
identical widening to `TextCard`/`AlbumCard`: the header now renders when `titleSlotVisible(title)`
OR `isPinned` OR `isFavorite` OR `imageCount > 0` (Text only) OR `reminderCount > 0`, with the title
`Text` itself made independently conditional so a blank title still renders no title text
(conditional-render-no-dead-space). Every existing call site keeps compiling and rendering
unchanged — the guard only widens what content survives a blank title, no signature changed.
Green-gate evidence: `testDebugUnitTest` for `TextCardTest` (12 tests) and `AlbumCardTest` (20
tests), both 0 failures — extended with source-structural assertions (the same
`CardBase`/`SwipeableActionRow` Robolectric-unrenderable blocker `TextCardImageIndicatorTest` /
`ReminderIndicatorCardWiringTest` document applies here too) proving the widened guard's exact
negated-condition text and that the `ReminderIndicator` call remains reachable, gated only on
`reminderCount > 0`, independent of title visibility. The rendered, on-device proof of a
title-less+reminder card's face is a Gate-2 device check (SecondBrain `155-12-SUMMARY.md`), not a
hub-side gate.

**JitPack resolution evidence:** `https://jitpack.io/com/github/Ygaray/yahirandroidtaste/v1.11.1/yahirandroidtaste-v1.11.1.pom`
